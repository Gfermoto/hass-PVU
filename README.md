...
| Параметр | `pvu_min.yaml` | `pvu.yaml` |
|---|---|---|
| Температурный контур | ✅ | ✅ |
| Вентилятор по IAQ | ✅ | ✅ |
| Антизаморозка | ✅ | ✅ |
| Anti-flap (min_interval) | ❌ | ✅ |
| Профили климата/жилья | ❌ | ✅ |
| Политики away=off | ❌ | ✅ |
| ECO режим | ❌ | ✅ |
...
Различия дефолтов объясняются тем, что `pvu.yaml` дополнительно корректирует пороги через `climate_profile`; для `pvu_min` используются «сырые» значения без профилей.

> **v0.7.0+**: параметр `temp_hysteresis_off` удалён из обоих вариантов. Если он был настроен — при обновлении blueprint поле просто исчезнет из UI без влияния на остальные настройки.
...
#### 1. Логика выбора режима HVAC

ПВУ — **вентиляционное** устройство, а не основная система отопления (тепло дают радиаторы/тёплый пол). Blueprint управляет **воздухообменом** как первичной задачей.

Приоритеты выбора режима:

| Условие | Режим |
|---|---|
| `away=off` | `off` |
| ECO активен (все датчики < ECO-порогов) | `off` |
| Мороз снаружи (`outdoor < outdoor_vent_min`) | `heat` — подогрев приточного воздуха |
| Жарко внутри + охлаждение разрешено | `cool` |
| Всё остальное (по умолчанию) | `fan_only` — вентиляция с рекуперацией |

- `fan_only` — режим по умолчанию: установка вентилирует постоянно, пока ECO или away=off не скажут иначе.
- `heat` — **подогрев приточного воздуха**, не нагрев помещения. Включается только при морозе снаружи.
- `cool` — охлаждение приточного воздуха. Включается только когда в помещении жарко (выше `target + temp_hysteresis_on`) и наружный воздух достаточно холодный.
- Уставка температуры (`temp_day`/`temp_night`/`temp_away`) отправляется как **setpoint приточного воздуха**.
- Команды применяются в два шага:
  1. `climate.set_temperature` (уставка, если изменилась, пропускается при `desired=off`);
  2. `climate.set_hvac_mode` (если целевой режим отличается от текущего).
- Для HVAC добавлен anti-flap: `hvac_min_switch_minutes` — минимальный интервал между переключениями.
...
#### 7. ECO режим (`pvu.yaml`)

- Включается параметром `eco_mode_enabled` (по умолчанию выключен).
- Если включён и все **активные** датчики воздуха ниже своих ECO-порогов → `hvac_mode=off`.
- Датчики, не сконфигурированные в основной секции, в ECO-проверке не участвуют.
- Если ни один датчик не задан — ECO не активируется (нет смысла выключать установку без какого-либо контроля воздуха).
- При ухудшении воздуха (хотя бы один датчик выше порога) включается стандартная логика (антизаморозка, `outdoor_temp_policy`, вентиляция).
- ECO применяется при `away_behavior=maintain`; если установка уже выключена по `away=off`, ECO не влияет.
- Использует существующий `hvac_min_switch_minutes` как anti-flap.
- Отдельная секция настроек «🍃 ECO режим»: `eco_co2_max`, `eco_pm25_max`, `eco_voc_max`, `eco_no_max`, `eco_humidity_max`.

#### 8. Диагностика (полная версия)
...
4. **Импорт Blueprint**:
  ...
  **Поведение blueprint** (`pvu.yaml`):
  - Установка вентилирует постоянно (`fan_only` по умолчанию), если не активен ECO или `away=off`.
  - При морозе снаружи (`outdoor < outdoor_vent_min`) переходит в `heat` — подогрев приточного воздуха.
  - Охлаждение включается только при жарком помещении + подходящей уличной температуре.
  - Скорость вентилятора (`low/medium/high`) управляется по CO2/PM2.5/VOC/NOx/влажности.
  - Наружные датчики качества воздуха ограничивают разгон скорости.
  - Антизаморозка через `outdoor_vent_min`, ECO через пороги IAQ-датчиков.
  - Реакция по изменению датчиков и presence + периодический пересчёт каждые 2 минуты.
