...
Blueprint для автоматизации приточно-вытяжной установки (ПВУ) Turkov в Home Assistant: температурный контур, качество воздуха, антизаморозка, политики в режиме отсутствия.

**Оглавление:** [Описание](#описание) · [Варианты blueprint](#варианты-blueprint) · [Быстрый старт](#быстрый-старт-3-шага) · [Возможности](#функциональные-возможности-актуально-для-pvuyaml) · [Установка](#установка-и-настройка) · [Разработка](#разработка-и-качество) · [Поддержка](#поддержка-и-обратная-связь)

---

### Описание

Готовое решение для автоматизации приточно-вытяжной вентиляции на базе Home Assistant и оборудования Turkov. Управление температурой и скоростью вентилятора по сценариям day/night/away, ограничения по наружной температуре, опциональные датчики CO2/PM2.5 и др. Два варианта: минимальный стабильный (`pvu_min.yaml`) и полный с профилями и диагностикой (`pvu.yaml`).
...
- `pvu_min.yaml` — минимальный стабильный контур (температура, вентилятор, антизаморозка).
- `pvu.yaml` — полная версия (профили климата/жилья, политики away, anti-flap, диагностика).
...
- Политика применения `away=off` для HVAC (`away_off_hvac_policy`):
  - `respect_min_interval` (по умолчанию): режим `off` подчиняется `hvac_min_switch_minutes`;
  - `immediate`: режим `off` применяется сразу, без ожидания anti-flap интервала.
- Политика управления вентилятором в `away=off` (`away_off_fan_policy`):
...
- Для `away=off` поведение anti-flap задаётся явно через `away_off_hvac_policy` (через интервал или сразу).
...
- Дополнительно доступна post-check верификация: после отправки команды можно проверить через `command_verify_delay_sec`, что фактический state совпал с целевым (полезно для диагностики device coupling/ограничений интеграции).
...
- Для **ручной карточки** используйте файлы `airflow_card.yaml` / `airflow_card_min.yaml` и вставляйте их в **Manual card**.
