...
- **ECO при недоступном датчике**: `eco_has_active_sensors` теперь проверяет наличие
  сконфигурированного entity (`'.' in co2_entity` и т.д.), а не его текущую активность.
  При кратком обрыве WiFi/MQTT (sensor → `unavailable`) датчик остаётся "учтённым",
  а индивидуальные `eco_*_ok` возвращают `True` (inactive → assumed OK), поэтому
  ECO корректно активируется вместо блокировки.
