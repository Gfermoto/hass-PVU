...
- **CO2 правдоподобность**: снижен минимальный порог с `>= 300` до `>= 200` ppm в обоих
  блюпринтах (`pvu.yaml`, `pvu_min.yaml`). Датчик SCD41 при прогреве или кратковременном
  переподключении мог возвращать значения 200–299 ppm, что вызывало `co2_active=False` →
  `eco_has_active_sensors=False` → ECO не активировался даже при чистом воздухе.
