...
ECO **не активируется**. Для включения ECO требуется хотя бы один активный датчик воздуха (CO2, PM2.5, VOC, NOx или влажность). Без датчиков `eco_has_active_sensors=false` и установка не выключается. Это защита от случая «включил галочку, ничего не настроил → установка постоянно выключена».
