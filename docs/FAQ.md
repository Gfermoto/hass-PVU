...
- режим поддерживается устройством (проверяется по `hvac_modes`/`fan_modes` из `state_attr`).
...
Используйте пошаговый гайд: [`TUNING.md`](./TUNING.md).
...
ECO режим доступен только в `pvu.yaml`. Если включён (`eco_mode_enabled=true`) и все **активные** датчики воздуха ниже своих ECO-порогов — blueprint выбирает `hvac_mode=off`.

Как только хотя бы один датчик превышает порог — включается стандартная логика (антизаморозка, ограничения по улице, температурный контур). ECO просто пропускает управление в нормальный поток.

## Что будет, если включить ECO режим, но датчики воздуха не настроены?

ECO **не активируется**. Для включения ECO требуется хотя бы один активный датчик воздуха (CO2, PM2.5, VOC, NOx или влажность). Без датчиков `eco_has_sensors=false` и установка не выключается. Это защита от случая «включил галочку, ничего не настроил → установка постоянно выключена».

## ECO и антизаморозка — как приоритеты?

Никакого конфликта нет. ECO выключает установку только когда воздух чистый. Когда воздух ухудшается, ECO отходит в сторону и работает стандартная логика — включая антизаморозку: если холодно снаружи, `fan_only` блокируется и при необходимости используется `heat`.

## ECO работает в режиме отсутствия?

Зависит от `away_behavior`:
- `maintain` — ECO применяется, как обычно.
- `off` — установка и так выключена по политике away; ECO не влияет.
