...
**[Здесь будет: схема трёх контуров — блок-схема или диаграмма «HVAC / Fan / Safety» и как они стыкуются в один вызов]**
...
**[Здесь будет: скриншот примера диагностического уведомления (persistent_notification) с режимами и причиной skip]**
...
**[Здесь будет: блок-схема/flowchart одного цикла — контекст → температура → ограничения → HVAC → Fan → применение]**
...
**[Здесь будет: опционально — схема сценария away: таймлайн «armed_away» → immediate vs respect_min_interval, или таблица политик]**
...
**Чеклист иллюстраций (вставить позже):**

1. Схема трёх контуров (HVAC / Fan / Safety) и их стыковка в один вызов  
2. Опционально: антизаморозка — температура за окном vs выбранный режим  
3. Скриншот диагностического уведомления (persistent_notification) с режимами и причиной skip  
4. Блок-схема одного цикла: контекст → температура → ограничения → HVAC → Fan → применение  
5. Скриншот мастера настройки automation (свёрнутые/развёрнутые секции blueprint)  
6. Скриншот дашборда или KPI-карточки (по желанию — график времени в зоне)  
7. Опционально: сценарий away — таймлайн или таблица политик
