...
Проект вырос в две ветки: `pvu_min.yaml` (стабильная база) и `pvu.yaml` (develop-версия с расширениями). В develop помимо fail-safe и `debug_mode` теперь есть профили Этапа 3: `climate_profile` (`continental`/`mild`/`cold`) и `home_profile` (`apartment`/`house`/`office`), а также готовые дашборды/карточки и KPI-пакет для проверки порогов на реальных данных.
...
- доводка `v1.0.0`: пройти полный release-checklist без компромиссов;
- сезонная проверка "зима/межсезонье/лето" как регулярный ритуал;
- сбор подтверждений стабильной работы от независимых пользователей.
...
- `pvu.yaml` — если хотите новые возможности и более удобный UI.
