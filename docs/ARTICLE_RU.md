...
Проект вырос в две ветки: `pvu_min.yaml` (стабильная база) и `pvu.yaml` (develop-версия с расширениями). В develop добавлены безопасная политика при неизвестной наружной температуре (`outdoor_temp_policy`), отладочный режим (`debug_mode`) и секции входных полей (`input sections`) в UI Home Assistant.
