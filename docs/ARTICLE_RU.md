...
Каждый запуск automation — один проход по цепочке:

1. **Контекст**: день или ночь, дома ли кто-то, валидны ли показания датчиков.
2. **Целевая температура**: `temp_day` / `temp_night` / `temp_away` в зависимости от профиля.
3. **Ограничения по улице**: разрешено ли охлаждение и вентиляция (`cool_allowed`, `vent_allowed`) с учётом `outdoor_vent_min` и `outdoor_temp_policy`.
4. **Режим HVAC** по приоритетам: сначала `off` (away + политика выключить), затем антизаморозка, охлаждение, вентиляция — только те режимы, что разрешены и поддерживаются устройством.
5. **Скорость вентилятора**: считается «насколько испортился воздух» по датчикам.
6. **Применение**: вызываются только те сервисы, которые реально нужны — режим или температура изменились.
...
На части установок команда «выключить вентилятор» (`fan_mode: off`) по сути гасит и весь HVAC — это аппаратная особенность устройства, не баг в логике. Побочный эффект: при постановке на охрану устройство может вести себя неочевидно (то выключится, то снова включится), минуя запланированную логику blueprint. В полной версии под это вынесены две отдельные политики.
